...
export function yearFourPlanetFiveQuestion() {
  let initialValue =
    randomNumberGenerator(1000) * 10 + randomNumberGenerator(9) + 1; // Generates a random number from 1 to 9999 which does not end in a zero
...
export function yearFourPlanetSixAnswer(values, playerAnswer) {
  ...
  if (
    (values[3] === "mm" && values[4] === "m") ||
    (values[3] === "ml" && values[4] === "l") ||
    (values[3] === "g" && values[4] === "kg")
  ) {
    correctAnswer *= 1000;
  } else {
    correctAnswer *= 100;
